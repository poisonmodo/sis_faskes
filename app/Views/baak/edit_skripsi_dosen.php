...
?>                         
                            <h3 class="card-title">Edit Dosen <?php echo $ket; ?></h3>
                        </div>
                        <!-- /.card-header -->
                        <div class="card-body">
                            <p class="card-text">
                        <?php   $err = session()->getFlashdata('errors'); 
                                ...
                                if(!isset($msg)) { ?>
                                <form method="post" action="<?php echo site_url('baak/skripsi/dosen/edit/'.$skripsi_id.'/'.$id) ?>">
...
<?php if($dosenlist) { 
		foreach($dosenlist as $dosen) { ?>
											<option value="<?php echo $dosen->nik ?>" <?php echo ($dosen->nik==$dosen_skripsi->nik)?"selected='selected'":"" ?> ><?php echo $dosen->nama_dosen ?></option>
<?php 	}
	  } ?>		
										</select>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <div class="form-group">
                                            <div class="form-check">
                                                <input class="form-check-input" type="radio" name="posisi" id="posisi" value="1" <?php echo ($dosen_skripsi->posisi==1)?"checked='checked'":"" ?>> 
                                                <label for="posisi" class="form-check-label">Pembimbing</label>
                                            </div>
                                            <div class="form-check">
                                                <input class="form-check-input" type="radio" name="posisi" id="posisi" value="2"  <?php echo ($dosen_skripsi->posisi==2)?"checked='checked'":"" ?>> 
                                                <label for="posisi" class="form-check-label" >Penguji I</label>
                                            </div>
											<div class="form-check">
                                                <input class="form-check-input" type="radio" name="posisi" id="posisi" value="3"  <?php echo ($dosen_skripsi->posisi==3)?"checked='checked'":"" ?>> 
									...
									<div class="form-group">
                                        ...
                                        <textarea name="deskripsi" class="form-control <?php echo (isset($err["deskripsi"])) ? "is-invalid" : "" ?>" id="deskripsi" placeholder="Silakan isi deskripsi"><?php echo $dosen_skripsi->deskripsi ?></textarea>
